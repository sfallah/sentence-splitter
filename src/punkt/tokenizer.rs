...
use crate::punkt::trainer::TrainingData;
...
///
/// ```
/// # use sentence_splitter::{SentenceByteOffsetTokenizer, TrainingData};
/// # use sentence_splitter::params::Standard;
/// #
  ...
  fn next(&mut self) -> Option<(usize, usize)> {
    while let Some((slice, tok_start, ws_start, slice_end, len)) = self.iter.next() {
      ...
      for mut t in WordTokenizer::<P>::new(slice) {
        ...
        crate::punkt::util::annotate_first_pass::<P>(&mut t, self.data);
...
///
/// ```
/// # use sentence_splitter::{SentenceTokenizer, TrainingData};
/// # use sentence_splitter::params::Standard;
/// #
/// let doc = "this is a great sentence! this is a sad sentence.";
/// let data = TrainingData::english();
    ...
    + DefinesSentenceEndings,
{
  ...
  fn next(&mut self) -> Option<(&'a str, usize, usize)> {
    self.iter.next().map(|(start, end)| (&self.doc[start..end], start, end))
  }
}
...
{
  use crate::punkt::prelude::{BEG_LC, MID_UC, ORT_LC, ORT_UC};
...
where
  P: DefinesPunctuation,
{
  use crate::punkt::prelude::ORT_LC;
